...
In this lab you will bootstrap the Kubernetes control plane. The following components will be installed on the `k8s-controller` machine: Kubernetes API Server, Scheduler, and Controller Manager.

## Prerequisites

Connect to the `jumpbox` and copy Kubernetes binaries and systemd unit files to the `k8s-controller` machine:
...
The commands in this lab must be run on the `k8s-controller` machine. Login to the `k8s-controller` machine using the `ssh` command. Example:

```bash
ssh root@k8s-controller
```
...
The commands in this section will affect the entire cluster and only need to be run on the `k8s-controller` machine.
...
```bash
curl --cacert ca.crt \
  https://k8s-controller.k8s.local:6443/version
```
