...
Generate a kubeconfig file for the `k8s-node-0` and `k8s-node-1` worker nodes:
...
```text
k8s-node-0.kubeconfig
k8s-node-1.kubeconfig
```
...
Copy the `kubelet` and `kube-proxy` kubeconfig files to the `k8s-node-0` and `k8s-node-1` machines:
