...
for HOST in k8s-node-0 k8s-node-1; do
  scp configs/kubelet-config.yaml root@${HOST}:~/
done
```
...
The commands in the next section must be run on each worker instance: `k8s-node-0`, `k8s-node-1`. Login to the worker instance using the `ssh` command. Example:
...
Be sure to complete the steps in this section on each worker node, `k8s-node-0` and `k8s-node-1`, before moving on to the next section.
...
k8s-node-0   Ready    <none>   1m     v1.32.3
k8s-node-1   Ready    <none>   10s    v1.32.3
```
